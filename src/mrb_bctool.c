/*
** mrb_bctool.c - BCTool class
**
** Copyright (c) Yannick Koechlin 2018
**
...
//takes mruby bytecode as a string and evaluates it
...
b_eval(mrb_state *mrb, mrb_value self)
{
  ...
  if (!irep) {
    mrb_raisef(mrb, E_SCRIPT_ERROR, "can't load mruby bytecode");
  }
...
void mrb_mruby_bctool_gem_init(mrb_state *mrb)
{
  struct RClass *bctool;
  bctool = mrb_define_class(mrb, "BCTool", mrb->object_class);
  mrb_define_class_method(mrb, bctool, "eval", b_eval, MRB_ARGS_ARG(1, 3));
  mrb_define_module_function(mrb, bctool, "compile", b_compile, MRB_ARGS_ARG(1, 3));

  DONE;
}

void mrb_mruby_bctool_gem_final(mrb_state *mrb)
{
}
